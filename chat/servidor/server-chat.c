...
void* manejar_cliente(void* arg) {
    ...
    while ((bytes = recv(sock, buffer, sizeof(buffer) - 1, 0)) > 0) {
        ...
        // Detección de comando especial
        if (strncmp(buffer, "enviar_archivo:", 15) == 0) {
            // Separar campos: archivo:destino:tamaño
            strtok(buffer, ":"); // "enviar_archivo"
            char* archivo = strtok(NULL, ":");
            char* destino = strtok(NULL, ":");
            char* tam_str = strtok(NULL, ":");
            
            printf("quien aceptó: %s\n", destino);

            if (!archivo || !destino || !tam_str) {
                send(sock, "Formato inválido. Usa enviar_archivo:nombreArchivo:nombreUsuario:tamaño\n", 70, 0);
                continue;
            }

            long tam_archivo = atol(tam_str);
            ...
            for (int i = 0; i < num_clientes; ++i) {
                if (strcmp(clientes[i].nombre, destino) == 0) {
                    ...
                    snprintf(noti, sizeof(noti),
                            "%s desea enviarte el archivo \"%s\" (%.2f KB). ¿Deseás recibirlo? (SI/NO)\n",
                            nombre, archivo, tam_archivo / 1024.0);
                    send(clientes[i].socket, noti, strlen(noti), 0);
                    encontrado = 1;
                    break;
                }
            }
            ...
            if (!encontrado) {
                ...
                send(sock, "Solicitud de envío enviada al receptor.\n", 40, 0);
            }

            continue;
        }

        char buffer_original[BUFFER_SIZE];
        strncpy(buffer_original, buffer, sizeof(buffer_original));
        buffer[bytes] = '\0';

        char* comando = strtok(buffer, ":");

        if (comando && strcmp(comando, "aceptar_archivo") == 0) {
            char* archivo = strtok(NULL, ":");
            char* emisor = strtok(NULL, ":");

            if (archivo && emisor) {
                pthread_mutex_lock(&mutex);
                int encontrado = 0;
                for (int i = 0; i < num_clientes; ++i) {
                    if (strcmp(clientes[i].nombre, emisor) == 0) {
                        char msg[BUFFER_SIZE];
                        snprintf(msg, sizeof(msg), "✅ %s aceptó recibir el archivo \"%s\". Iniciá la transferencia.", nombre, archivo);
                ...
                pthread_mutex_unlock(&mutex);
                if (!encontrado) {
                    send(sock, "Emisor no encontrado.\n", 23, 0);
                }
            }
            continue;
        }

        if (comando && strcmp(comando, "rechazar_archivo") == 0) {
            char* archivo = strtok(NULL, ":");
            char* emisor = strtok(NULL, ":");
            ...
            if (archivo && emisor) {
                ...
                for (int i = 0; i < num_clientes; ++i) {
                    if (strcmp(clientes[i].nombre, emisor) == 0) {
                        char msg[BUFFER_SIZE];
                        snprintf(msg, sizeof(msg), "❌ %s rechazó recibir el archivo \"%s\".", nombre, archivo);
                        send(clientes[i].socket, msg, strlen(msg), 0);
                        encontrado = 1;
                        break;
                    }
